...
export default class HangXin implements IAction {
    ...
    //发票查询
    fpcx = (url, data) => {
        let s1 = "Select FPZL 发票种类,FPDM 发票代码,FPHM 发票号码,KPJH 开票机号,KPRQ 开票日期,XSDJBH 单据号,"
            + "GFMC 购方名称,GFSH 购方税号,GFDZDH 地址电话,GFYHZH 银行账号,MW 密文,HJJE 合计金额,HJSE 合计税额,"
            + "BZ 备注,KPR 开票人,SKR 收款人,FHR 复核人,QDBZ 清单标志,ZFBZ 作废标志 "
            + "From XXFP Where KPRQ > '2017-10-10'";
     ...
     * 导入发票
     */
    dru = async (url: string, data: InvoiceMap) => {
        const s2 = '||';   /////////字段分隔符
        let s1;
        let res;
        let response;
        ...
        //注意： 明细条数、总金额、总税额
        s1 = (data.goodsInfos && data.goodsInfos.length) + s2 + 'X150208001' + s2 + '123' + s2
            + data.buyerName + s2 + data.buyerTaxID + s2
            + data.buyerAddrTel + s2 + data.buyerFinancialAccount + s2
            + data.taxExclusiveTotalAmount + s2 + data.taxTotalAmount + s2 + '0.16' + s2
            + data.payee + s2 + data.checker + s2
            + data.sellerFinancialAccount + s2 + '转子铁芯' + s2 + (data.goodsInfos && data.goodsInfos.length);
        //2 
        ...
        items.forEach(async (item, index) => {
            s1 = '001' + s2 + item.item + s2 + item.specification + s2 + item.measurementDimension + s2
                + item.quantity + s2 + item.price + s2 + '3' + s2
                + item.amount + s2 + item.taxScheme + s2 + item.taxAmount + s2
                + '0' + s2 + (index + 1) + s2 + '4001' + s2 + '1090602050000000000';
