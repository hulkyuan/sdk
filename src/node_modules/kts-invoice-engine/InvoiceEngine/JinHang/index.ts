...
export default class HangXin implements IAction {
    ...
    dru = async (url: string, data: InvoiceMap) => {
        ...
        s1 = (data.goodsInfos && data.goodsInfos.length) + s2 + 'X150208001' + s2 + '123' + s2
            ...
            + data.sellerFinancialAccount + s2 + '转子铁芯' + s2 + '8';
        //2 
        ...
        for (let i = 0; i < items.length; i++) {
            s1 = '001' + s2 + items[i].item + s2 + items[i].specification + s2 + items[i].measurementDimension + s2
                + items[i].quantity + s2 + items[i].price + s2 + '3' + s2
                + items[i].amount + s2 + items[i].taxScheme + s2 + items[i].taxAmount + s2
                + '0' + s2 + (i + 1) + s2 + '4001' + s2 + '1090602050000000000';
