...
export default class HangXin implements IAction {
    ...
    fphm = (url) => {
        return this.send(url, this.mockCurrent);
    }
    //返回样本：
    // <?xml version="1.0" encoding="utf-8"?>
    ...
    fpcx = (url, data) => {
        return this.send(url, this.mockChaxun);
    }
    send = (url, data): Promise<any> => {
        // 真实开票用到的请求，将js对象转换xml字符串
        const reqXml = builder.create(toAt(transformData('jinshui', data))).end({ pretty: true });
